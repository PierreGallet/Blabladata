...
def formating_sentiment_imdb(path_input='./data/a/fr', path_output='./data/sentiment/sentiment_imdb_fr.csv'):

    with open(path_output, 'w+') as output:
        fieldnames = ['label', 'sentence']
        writer = csv.DictWriter(output, fieldnames=fieldnames, delimiter=';')
        writer.writeheader()
        for directory in glob.glob(path_input+'/*'):
            for directory2 in glob.glob(directory+'/*'):
                files = os.listdir(directory2)
                print('dealing with', directory2)
                for i in range(len(files)):
                    with open(directory2+'/'+files[i]) as f:
                        sentence = re.sub(r'<.*?>', '', f.read())
                        if directory2.split('/')[-1] == 'pos_fr':
                            label = str(1)
                        else:
                            label = str(0)
                        writer.writerow({'label': label, 'sentence': sentence})

def formating_wiki(path, path_output):
    with open(path, 'r+') as f:
        with open(path_output, 'w+') as output:
            for line in f.readlines():
                if not re.match(r'^\s*$', line) and len(line) > 55:
...
if __name__ == '__main__':
    ...
    path = './data/Wikipedia/dataset_AA.txt'
    path_output = './data/Wikipedia/wiki_french.txt'
    formating_wiki(path, path_output)
