...
class word2vec_paraphrase():
    ...
    def train(self, path_sentences, size=128, window=5, min_count=2):
        """
        ...
        model.save('./paraphrase_detection/tmp/word2vec')
    ...
    def get_dictionaries(self):
        ...
        model = Word2Vec.load('./paraphrase_detection/tmp/word2vec')
        ...
        with open('./paraphrase_detection/tmp/index_dict.pk', 'wb') as f:
            pickle.dump(index_dict, f)
        with open('./paraphrase_detection/tmp/word_vectors.pk', 'wb') as f:
    ...
    def get_weights_matrix(self):
        ...
        with open('./paraphrase_detection/tmp/embedding_weights.pk', 'wb') as f:
    ...
    def format_input(self):
        ...
        self.path_sentences_output_1 = self.directory+'/input/word2vec/sentences_1.npy'
        self.path_sentences_output_2 = self.directory+'/input/word2vec/sentences_2.npy'
        self.path_labels_output = self.directory+'/input/word2vec/labels.npy'

        with open(self.path_sentences_1, 'r+') as f:
            lines = f.readlines()
            max_lenght = max([len(line.split()) for line in lines])
            sentences_1 = np.zeros((len(lines), max_lenght))   # size = samples x max lenght of sentences
            i = 0
            nb_unknown = 0
            nb_token = 0
            for line in lines:
                ...
                sentences_1[i, :lenght] = sentence_formated[:lenght]
                i += 1
        ...
        with open(self.path_sentences_2, 'r+') as f:
            lines = f.readlines()
            max_lenght = max([len(line.split()) for line in lines])
            sentences_2 = np.zeros((len(lines), max_lenght))   # size = samples x max lenght of sentences
            i = 0
            nb_unknown = 0
            nb_token = 0
            for line in lines:
                ...
                for word in line.split():
                    ...
                    try:
                        ...
                        nb_unknown += 1
                lenght = len(sentence_formated)
                sentences_2[i, :lenght] = sentence_formated[:lenght]
                i += 1
        print('there was', nb_unknown, 'unknown tokens out of', nb_token, 'total tokens, which account for', int((float(nb_unknown) / float(nb_token))*100), '% of all tokens')
        ...
        with open(self.path_sentences_output_1, 'wb') as f:
            np.save(f, sentences_1)
        with open(self.path_sentences_output_2, 'wb') as f:
            np.save(f, sentences_2)
        with open(self.path_labels_output, 'wb') as f:
            np.save(f, labels)

        print('shape of sentences (nb_sample, max_len):', sentences_1.shape)
